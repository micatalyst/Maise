...
export default function Video_Forms_Step1({ handleNextStep, original_content_file, setOriginal_content_file, original_content_PreviewUrl, setOriginal_content_PreviewUrl }) {
  const title = useSelector((state) => state.TempVideoContentSlice.title || '');
  const original_content_category = useSelector((state) => state.TempVideoContentSlice.original_content_category || '');
  const original_content_language = useSelector((state) => state.TempVideoContentSlice.original_content_language || '');
  const description = useSelector((state) => state.TempVideoContentSlice.description);
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop,
    accept: {
      // Temporário até ver os formatos suportados
      'video/mp4': ['.mp4'],
      'video/mpeg': ['.mpeg'],
      'video/x-msvideo': ['.avi'],
    },
            ...
            >
              <div className="file-uploaded-zone">
                <div className="file-side-bar">
                  ...
                  <p className="video">MP4 / MPEG / AVI</p>
                  <button
